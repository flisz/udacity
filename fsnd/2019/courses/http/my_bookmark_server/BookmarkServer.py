...
def CheckURI(uri, timeout=5):
    ...
    try:
        ...
        print('while trying to get URI: {} ran into exception: {}'.format(uri,e))
        return False

    print("{} type {}".format(r.status_code,type(r.status_code)))
...
class Shortener(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_header('content-type','text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write('more params required!! need both shortname and longuri'.encode())
        else:
            longuri = params.get("longuri",[""])[0]
            shortname = params.get("shortname",[""])[0]
            if CheckURI(longuri):
...
if __name__ == '__main__':
    ...
    print(f'serving forever at {server_address}')
    httpd.serve_forever()
